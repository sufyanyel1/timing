...
// Change the route to use query parameters
app.get("/time", (req, res) => {
    // Get the times from query parameters
    let times = req.query.times || '';
    console.log("TIMES", times);
    
    // Split by comma and trim whitespace
    let timesArray = times.split(',').map(time => time.trim()).filter(time => time !== '');
    console.log("timesArray", timesArray);
    
    // Check if timesArray is empty
    if (!timesArray.length) {
        res.status(400).send("Please enter Time!");
        return;
    }

    // Calculate the time inside
    const result = calculateTimeInside(timesArray);
    console.log(`Total time inside: ${result.totalTime}`);
    console.log(`You will complete 8 hours and 30 minutes by: ${result.completionTime}`);
...
// Function to calculate time inside
function calculateTimeInside(times) {
    const inOutPairs = [];
